...
describe('Replication / Change APIs', function() {
  ...
  describe('Model.replicate(since, targetModel, options, callback)', function() {
    ...
    it('detects 3rd-party UPDATE made during replication', function(done) {
      async.series([
        function createModel(next) {
          SourceModel.create({ id: '1' }, next);
        },
        function replicate(next) {
          SourceModel.replicate(TargetModel, next);
        ...
        function replicateWith3rdPartyModifyingData(next) {
          ...
          TargetModel.bulkUpdate = function(data, cb) {
            var self = this;
            // Simulate the situation when a 3rd party modifies the instance
            // to be updated while a replication run is in progress,
            // where the 3rd-party does not update change-tracking records.
