...
PersistedModel.bulkUpdate = function(updates, callback) {
  ...
  updates.forEach(function(update) {
    ...
    switch (update.type) {
      case Change.UPDATE:
        tasks.push(function(cb) {
          applyUpdate(Model, id, update.data, update.change, conflicts, cb);
        });
        break;

      case Change.CREATE:
        tasks.push(function(cb) {
          applyCreate(Model, id, update.data, update.change, conflicts, cb);
        });
        break;
      case Change.DELETE:
...
function applyUpdate(Model, id, data, change, conflicts, cb) {
  ...
  Model.findById(id, function(err, inst) {
    ...
    Model.updateAll(inst.toObject(), data, function(err, count) {
      if (err) return cb(err);
      if (count === 0) {
...
function applyCreate(Model, id, data, change, conflicts, cb) {
